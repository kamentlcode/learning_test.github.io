...
function foot() {
	...
	if(!is_mobile()) {
		document.writeln("<p>本站小说为转载作品，所有章节均由网友上传，转载至本站只是为了宣传本书让更多读者欣赏。</p>");
	}
	document.writeln("<p>Copyright &#169; " + year + " <a href=\"http://www.yisvip.com/\">svip</a>(" + location.host + ") All Rights Reserved. 沪ICP备19819998号</p>");
	document.writeln("</footer>");
...
function BookVote(article_id) {
	$.get('/modules/article/uservote.php?id=' + article_id + '&ajax_request=1', function(data) {
		showMsg(data);
	});
}
